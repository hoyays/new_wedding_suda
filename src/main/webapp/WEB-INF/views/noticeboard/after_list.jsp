	...
	$('#regist_btn').click(function() {
		if(${session_flag==fail}){
			alert('로그인 후 이용가능합니다.')
					...
					top.location.href="./afterwrite_view";
		}
...
<div class="cm_board_normal">
	...
	<tbody>
     ...
     <fmt:formatDate var="today" value="${now}" pattern="yyyy-MM-dd"/>
			...
			<fmt:formatDate var="today1" value="${dto.bdate}" pattern="yyyy-MM-dd"/>
			<fmt:formatDate var="today2" value="${dto.bdate}" pattern="HH:mm"/>		
			<tr>
				<td style="font-size:14px;">${dto.bid}</td> 
				       
				<td class="title"><a href="afterContent_view?category=${map.category }&search=${map.search }&page=${map.page}&bid=${dto.bid }" class="view_article" style="font-size:14px;">${dto.btitle }
				</a><c:if test="${today== today1}">  &nbsp;&nbsp;<img src="../images/icon_new.gif" width="52" height="18" alt="NEW" title="NEW" border="0" align="absmiddle" class="ICON_NEW"> </c:if>
               </td>        
               <td style="font-size:14px;">${dto.nname }</td>
               <c:choose>
               	<c:when test="${today== today1}">
               		<td style="font-size:14px;">${today2}</td>
               	</c:when>
               	<c:otherwise>
			   		<td style="font-size:14px;">${today1}</td>
               	</c:otherwise>
               </c:choose>
...
<div id="board_link_num"> 		
            <a href="./after_list?category=${map.category }&search=${map.search }&page=1" title="1 페이지">&lt;&lt;</a>
<c:choose>
<c:when test="${map.page<=1 }">
<a>&lt;</a>
</c:when>
<c:otherwise>
<a href="./after_list?category=${map.category }&search=${map.search }&page=${map.page-1}" title="이전">&lt;</a>
</c:otherwise>
</c:choose>
<c:forEach var="nowpage" begin="${map.startpage }" end="${map.endpage }">
	...
	<c:otherwise>
		<a href="./after_list?category=${map.category }&search=${map.search }&page=${nowpage}" title="2 페이지">${nowpage}</a>
	</c:otherwise>
	</c:choose>
</c:forEach>
...
<a>&gt;</a>        </c:when>
               <c:otherwise>
<a href="./after_list?category=${map.category }&search=${map.search }&page=${map.page+1}" title="다음" class="num_right">&gt;</a>
        </c:otherwise>
      </c:choose>
<a href="./after_list?category=${map.category }&search=${map.search }&page=${map.maxpage }" title="11 페이지" class="num_right">&gt;&gt;</a>
...
<div id="board_bottom">
	<form id="board_bottom_bx" action="./after_list" name="search" method="post">
		<ul> 
